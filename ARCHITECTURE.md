...
- **Lagring & driftverktyg 2026-07-17** – filbaserad store (`src/store/file.ts`, `./.data`, gitignorerad) när DATABASE_URL saknas utanför serverless: lokala körningar (svar, kommentarer, samtycken, AI-analyser, adminpublicering) överlever omstart; verifierad end-to-end med skarpt Opus-anrop → omstart → DSAR-export → radering. Boot-vakten tillåter staging utan databas/Upstash när ALLOW_DRAFT_CATALOG=true. Nya verktyg: `npm run validate:scales -- --anvandardata` (väljarreliabilitet på lagrade kanoniska svar, parvis null-hantering), `npm run audit:neutrality` (Monte Carlo-neutralitetsaudit mot publicerad katalog: hybrid halverar mittenskevheten jämfört med ren city-block), `npm run loadtest` (lokal prod-build: ~1 500 req/s, p95 26 ms vid 25 samtidiga, 0 fel).
- **Katalogcache + pooler-incident 2026-07-17** – loadActiveDataset cachar datasetet per instans (TTL 60 s): utan cache traffas databasen pa varje /kompass- och /api/analyze-request, och Supabase session-pooler (15 klienter) mattades under en admin-synk nar frysta serverless-instanser holl anslutningar som aldrig idle-stangs (EMAXCONNSESSION, ~4 min 500 pa /kompass; atgard: omdeploy recyklar instanser + cachen infordes). Adminsynk mot prod ska koras strikt sekventiellt: anvand scripts/syncCatalog.mjs (avstammande, bevarar kureringen, seedar bara vid behov). Rekommenderad infra-hardning (anvandaren): byt DATABASE_URL till Supabase transaction-pooler (port 6543) och/eller PGPOOL_MAX=1.
- **175 enhetstester** (`npm test`), **produktionsbuild grön** (`npm run build`). Kör lokalt: `npm run dev` → http://localhost:3000.
